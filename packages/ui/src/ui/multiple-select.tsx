          ...
          >
            ...
            >
              <TagList
                ...
                items={
                  [...selectedKeys].map(
                    (key): { id: Key; textValue: string } => ({
                      id: key,
                      textValue: parsedItems.find((item) => item.id === key)
                        ?.textValue as string,
                    }),
                  ) satisfies { id: Key; textValue: string }[]
                }
              >
            ...
            >
              <div className="flex w-full flex-row items-center justify-between px-2">
                ...
                <Button
                  ...
                  aria-label="Chevron"
                  className="ml-auto inline-flex items-center justify-center rounded-lg text-muted-foreground outline-hidden"
                >
                  <IconChevronDown
                    className={twMerge(
                      'group-has-open:-rotate-180 size-4 transition',
                    )}
                  />
                </Button>
              </div>
              ...
              >
                <ListBox
                  ...
                  {...props}
                >
                  {(filteredChildren as { id: Key; textValue: string }[])?.map(
                    (item) => (
                      <MultipleSelect.Item
                        key={item.id}
                        id={item.id}
