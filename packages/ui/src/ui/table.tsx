...
import { IconChevronDown, IconMenu2 } from 'ancient-icons';
import type {
  CellProps,
  ColumnProps,
  ColumnResizerProps,
        ...
        {props.allowResize ? (
          <ResizableTableContainer>
            <TablePrimitive
              {...props}
              className={twMerge(
                'table w-full min-w-full caption-bottom border-spacing-0 text-sm outline-hidden [--table-selected-background:color-mix(in_oklab,var(--color-primary)_5%,white_90%)] **:data-drop-target:border **:data-drop-target:border-primary dark:[--table-selected-background:color-mix(in_oklab,var(--color-primary)_25%,black_70%)]',
                className,
              )}
            >
              {children}
            ...
            {allowsSorting && (
              <span
                className={twMerge(
                  ...
                  isHovered ? 'bg-secondary-foreground/10' : '',
                  className,
                )}
              >
                <IconChevronDown
...
const TableRow = <T extends object>({
  ...
  return (
    <Row
      ...
      {...props}
      className={twMerge(
        'tr group relative cursor-default border-b bg-background selected:bg-(--table-selected-background) text-muted-foreground outline-hidden ring-primary selected:hover:bg-(--table-selected-background)/70 focus:ring-0 focus-visible:ring-1 dark:selected:hover:bg-[color-mix(in_oklab,var(--color-primary)_30%,black_70%)]',
        ((props.href && !props.isDisabled) || props.onAction) &&
          'cursor-pointer hover:bg-secondary/50 hover:text-secondary-foreground',
        className,
      )}
    >
      {allowsDragging && (
        <Cell className="group cursor-grab pr-0 ring-primary data-dragging:cursor-grabbing">
          <Button
            className="relative bg-transparent py-1.5 pl-3.5 pressed:text-foreground text-muted-foreground"
            slot="drag"
          >
            <IconMenu2 />
          </Button>
