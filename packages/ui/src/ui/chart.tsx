...
const ChartTooltipContent = ({
  ...
  return (
    <div
      ref={ref}
      className={twMerge(
        'grid min-w-[12rem] items-start gap-1.5 rounded-lg border bg-overlay px-3 py-2 text-overlay-foreground text-xs shadow-xl',
        className,
      )}
    >
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className="flex items-center gap-1.5 *:data-[slot=icon]:size-3 *:data-[slot=icon]:text-muted-foreground"
          >
