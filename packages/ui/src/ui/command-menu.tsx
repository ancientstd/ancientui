...
import { IconSearch } from 'ancient-icons';
    ...
    >
      ...
      >
        ...
        >
          <Modal
            className={twMerge([
              'fixed top-auto bottom-0 left-[50%] z-50 grid h-[calc(100vh-30%)] w-full max-w-full translate-x-[-50%] gap-4 overflow-hidden rounded-t-2xl bg-overlay text-overlay-foreground shadow-lg ring-1 ring-foreground/10 sm:top-[6rem] sm:bottom-auto sm:h-auto sm:w-full sm:max-w-2xl sm:rounded-xl dark:ring-border forced-colors:border',
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        className="w-full min-w-0 bg-transparent px-2.5 py-2 text-base text-foreground placeholder-muted-foreground outline-hidden focus:outline-hidden sm:text-sm [&::-ms-reveal]:hidden [&::-webkit-search-cancel-button]:hidden"
      />
      {escapeButton && (
        <Button
          onPress={() => state?.close()}
  ...
  CollectionRoot(props) {
    if (props.collection.size === 0) {
      return (
        ...
        <div
          role="menuitem"
          className="col-span-full p-4 text-center text-muted-foreground text-sm"
        >
          No results found.
        </div>
      );
