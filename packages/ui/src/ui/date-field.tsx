    ...
    >
      ...
      <FieldGroup>
        {prefix && typeof prefix === 'string' ? (
          <span className="ml-2 text-muted-foreground">{prefix}</span>
        ) : (
          prefix
        )}
        <DateInput />
        {suffix ? (
          typeof suffix === 'string' ? (
            <span className="mr-2 text-muted-foreground">{suffix}</span>
          ) : (
            suffix
          )
        ) : null}
...
const segmentStyles = tv({
  base: 'inline shrink-0 rounded p-0.5 type-literal:px-0 text-foreground tabular-nums tracking-wider caret-transparent outline-0 forced-color-adjust-none sm:text-sm forced-colors:text-[ButtonText]',
  variants: {
    isPlaceholder: {
...
const DateInput = ({
  ...
  return (
    <DateInputPrimitive
      className={composeTailwindRenderProps(
        className,
        'bg-transparent p-2 text-base text-foreground placeholder-muted-foreground',
      )}
      {...props}
    >
