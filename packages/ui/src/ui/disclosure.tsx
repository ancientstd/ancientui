...
import { IconChevronLeft } from 'ancient-icons';
import type {
  ...
  return (
    <Heading>
      <Button
        ...
        className={composeTailwindRenderProps(
          className,
          'group/trigger [&[aria-expanded=true]_[data-slot=disclosure-chevron]]:-rotate-90 **:data-[slot=icon]:-mx-0.5 flex w-full items-center justify-between gap-x-2 py-3 text-left font-medium open:text-foreground focus:text-foreground focus:outline-hidden disabled:cursor-default disabled:opacity-50 **:data-[slot=disclosure-chevron]:size-5 **:data-[slot=icon]:shrink-0 **:data-[slot=icon]:text-muted-foreground sm:text-sm forced-colors:disabled:text-[GrayText] **:[span]:flex **:[span]:items-center **:[span]:gap-x-1 **:[span]:*:data-[slot=icon]:mr-1',
        )}
        {...props}
      >
        {(values) => (
...
const DisclosurePanel = ({
  ...
  return (
    <CollapsiblePanel
      ...
      className={composeTailwindRenderProps(
        className,
        'overflow-hidden text-muted-foreground text-sm transition-all **:data-[slot=disclosure-group]:border-t **:data-[slot=disclosure-group]:**:[.internal-chevron]:hidden has-data-[slot=disclosure-group]:**:[button]:px-4',
      )}
      {...props}
    >
      <div
