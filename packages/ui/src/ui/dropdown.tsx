...
import { IconCheck } from 'ancient-icons';
import type {
  ListBoxItemProps,
  SectionProps,
  SeparatorProps,
...
const dropdownItemStyles = tv({
  base: [
    ...
    'group relative cursor-default select-none rounded-[calc(var(--radius-lg)-1px)] px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] forced-color:text-[Highlight] text-base text-foreground outline-0 forced-color-adjust-none sm:text-sm/6 forced-colors:text-[LinkText]',
    ...
    'data-danger:**:data-[slot=icon]:text-danger/60 **:data-[slot=icon]:size-4 **:data-[slot=icon]:shrink-0 **:data-[slot=icon]:text-muted-foreground focus:data-danger:**:data-[slot=icon]:text-danger',
...
const dropdownSectionStyles = tv({
  slots: {
    ...
    header:
      'col-span-full px-2.5 py-1 font-medium text-muted-foreground text-sm sm:text-xs',
  },
});
...
const DropdownKeyboard = ({
  ...
  return (
    <Keyboard
      classNames={{
        base: twMerge(
          'absolute right-2 group-hover:text-primary-foreground group-focus:text-primary-foreground pl-2',
          className,
        ),
      }}
      {...props}
