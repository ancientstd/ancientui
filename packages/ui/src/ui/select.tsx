...
import { IconChevronDown } from 'ancient-icons';
import type {
  ListBoxProps,
  PopoverProps,
  ...
  return (
    ...
    >
      ...
      <SelectValue
        ...
        className="*:data-[slot=icon]:-mx-0.5 *:data-[slot=avatar]:-mx-0.5 *:data-[slot=avatar]:*:-mx-0.5 grid flex-1 grid-cols-[auto_1fr] items-center text-base data-placeholder:text-muted-foreground *:data-[slot=avatar]:*:mr-2 *:data-[slot=avatar]:mr-2 *:data-[slot=icon]:mr-2 sm:text-sm [&_[slot=description]]:hidden"
      />
      <IconChevronDown
        aria-hidden
        className="size-4 shrink-0 text-muted-foreground duration-300 group-disabled:opacity-50 group-data-open:rotate-180 group-data-open:text-foreground forced-colors:text-[ButtonText] forced-colors:group-disabled:text-[GrayText]"
      />
    </Button>
  );
};
