    ...
    <SidebarContext value={contextValue}>
      <div
        className={twMerge(
          ...
          'group/sidebar-root has-data-[sidebar-intent=inset]:bg-sidebar dark:has-data-[sidebar-intent=inset]:bg-background',
          className,
        )}
        ref={ref}
        {...props}
...
const sidebar = tv({
  ...
  variants: {
    ...
    intent: {
      float:
        'bg-background p-2 group-data-[sidebar-collapsible=dock]/sidebar-container:w-[calc(var+theme(spacing.4)+2px)]',
      inset: [
        'bg-sidebar p-2 group-data-[sidebar-collapsible=dock]/sidebar-container:w-[calc(var(--sidebar-width-dock)+theme(spacing.2)+2px)] dark:bg-background',
      ],
      fleet: [
        'group-data-[sidebar-collapsible=dock]/sidebar-container:w-(--sidebar-width-dock)',
...
const Sidebar = ({
  ...
  return (
    <div
      ...
      className="group/sidebar-container peer hidden text-sidebar-foreground md:block"
      {...props}
    >
...
const footer = tv({
  base: [
    ...
    '**:data-[slot=menu-trigger]:hover:bg-(--sidebar-accent) **:data-[slot=menu-trigger]:hover:text-foreground',
  ],
  variants: {
    expanded: {
      true: '',
  ...
  return (
    ...
    >
      {state !== 'collapsed' && 'title' in props && (
        <Header className="group-data-[sidebar-collapsible=dock]/sidebar-container:-mt-8 mb-1 flex shrink-0 items-center rounded-md px-2.5 font-medium text-sidebar-foreground/70 text-xs outline-none ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 *:data-[slot=icon]:size-4 *:data-[slot=icon]:shrink-0 group-data-[sidebar-collapsible=dock]/sidebar-container:opacity-0">
          {props.title}
        </Header>
      )}
...
const sidebarItemStyles = tv({
  base: [
    'group/sidebar-item relative col-span-full cursor-pointer overflow-hidden rounded-lg px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] text-sidebar-foreground/70 focus-visible:outline-hidden sm:text-sm/6',
  ...
  variants: {
    ...
    isCurrent: {
      true: 'bg-(--sidebar-accent) text-foreground hover:bg-(--sidebar-accent)/90 hover:text-foreground **:data-[slot=menu-trigger]:from-(--sidebar-accent) **:data-[slot=icon]:text-foreground [&_.text-muted-foreground]:text-foreground/80',
    },
    isActive: {
      true: 'bg-(--sidebar-accent) text-sidebar-foreground **:data-[slot=menu-trigger]:flex',
    },
    isDisabled: {
  ...
  return (
    <main
      ...
      className={twMerge(
        ...
        'bg-background peer-data-[sidebar-intent=inset]:overflow-hidden dark:peer-data-[sidebar-intent=inset]:bg-sidebar',
...
const sidebarDisclosureTrigger = tv({
  base: [
    'group relative flex w-full cursor-pointer items-center overflow-hidden rounded-lg px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] text-sidebar-foreground/70 outline-hidden sm:text-sm/6',
...
const SidebarTrigger = ({
  ...
  return (
    ...
    >
      {children || (
        <>
          <IconLayoutSidebarRightExpandFilled className="hidden md:inline" />
          <IconMenu2 className="inline md:hidden" />
...
const nav = tv({
  base: [
    'isolate flex h-[3.2rem] items-center justify-between gap-x-2 px-4 text-navbar-foreground sm:justify-start md:w-full',
    'group-has-data-[sidebar-intent=default]/sidebar-root:border-b group-has-data-[sidebar-intent=fleet]/sidebar-root:border-b group-has-data-[sidebar-intent=default]/sidebar-root:bg-background',
  ],
  variants: {
    isSticky: {
