...
const Card = ({
  ...
  return (
    <div
      data-slot="card"
      className={twMerge(
        'group/card flex flex-col gap-(--card-spacing) rounded-lg border bg-background py-(--card-spacing) text-foreground shadow-xs [--card-spacing:theme(spacing.6)] has-[table]:overflow-hidden **:data-[slot=table-header]:bg-muted/50 has-[table]:**:data-[slot=card-footer]:border-t **:[table]:overflow-hidden',
        className,
      )}
      {...props}
    />
